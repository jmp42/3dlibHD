...
void S3L_drawTriangle(S3L_Vec4 point0,
  ...
  S3L_Index triangleIndex, uint8_t u8_page_index);
...
//  uint8_t S3L_zBuffer[S3L_MAX_PIXELS];
    uint8_t S3L_zBuffer[S3L_RESOLUTION_X * S3L_PAGE_Y];
...
static inline int8_t S3L_zTest(
  ...
  S3L_Unit depth)
{
  uint32_t index = (y%S3L_PAGE_Y) * S3L_RESOLUTION_X + x;
...
{
#if S3L_Z_BUFFER
  return S3L_zBuffer[(y%S3L_PAGE_Y) * S3L_RESOLUTION_X + x];
#else
  S3L_UNUSED(x);
  S3L_UNUSED(y);
...
{
#if S3L_Z_BUFFER
  S3L_zBuffer[(y%S3L_PAGE_Y) * S3L_RESOLUTION_X + x] = value;
#else
  S3L_UNUSED(x);
  S3L_UNUSED(y);
  S3L_UNUSED(value);
...
{
#if S3L_Z_BUFFER
  for (uint32_t i = 0; i < S3L_RESOLUTION_X * S3L_PAGE_Y; ++i)
...
//  endY = S3L_min(endY, S3L_RESOLUTION_Y );
  endY = S3L_min(endY, u8_page_index*S3L_PAGE_Y+S3L_PAGE_Y-1 );
...
//    if (currentY >= 0)
    if (currentY >= u8_page_index*S3L_PAGE_Y)
            /* clipping of pixels whose y < 0 (can't be easily done
...
#if S3L_Z_BUFFER
      uint32_t zBufferIndex = (p.y%S3L_PAGE_Y) * S3L_RESOLUTION_X + lXClipped;
#endif
...
//#include <qDebug>
void print_mat(S3L_Mat4 m, char* pcMatrixName)
{
//    qDebug() << "Matrix " << pcMatrixName;
...
void S3L_makeCameraMatrix(S3L_Transform3D cameraTransform, S3L_Mat4 m)
{
  ...
  S3L_makeRotationMatrixZXY(
    ...
    r);
//qDebug() << "cameta transformation " << cameraTransform.rotation.x << cameraTransform.rotation.y << cameraTransform.rotation.z,
//  print_mat(r, "r norm");
...
/* JMP : transformation / draw separation
 * WARNING : not usable with SORT option !
 * This mode only support 1 model in the scene !!!
*/
#define MAX_TRANSFORMED_VERTEX_COUNT    128
S3L_Vec4 transformed_vertex[MAX_TRANSFORMED_VERTEX_COUNT]; // transform once, render 8 th

void S3L_transformScene( S3L_Scene scene )
{
    S3L_Mat4 matFinal, matCamera;
    const S3L_Model3D *model;
    S3L_Index modelIndex, vertexIndex;
    ...
    S3L_Index vertexCount = scene.models[modelIndex].vertexCount;
    if ( vertexCount > MAX_TRANSFORMED_VERTEX_COUNT )
        vertexCount = MAX_TRANSFORMED_VERTEX_COUNT;
    model = &(scene.models[modelIndex]);
    for ( vertexIndex = 0 ; vertexIndex < vertexCount ; vertexIndex++ )
    {
        S3L_Vec4 *result = &transformed_vertex[vertexIndex]; // facilities
        result->x = model->vertices[3*vertexIndex + 0];
        result->y = model->vertices[3*vertexIndex + 1];
        result->z = model->vertices[3*vertexIndex + 2];
        result->w = S3L_FRACTIONS_PER_UNIT; // needed for translation
...
/*
 * for computation optiimsation :
 * rentder 8 times the same scene for video mode with blocks
 * S3L_transformScene must be called first once
 *
 */
void S3L_drawTransformedScene(S3L_Scene scene, uint8_t u8_page_index)
{
  const S3L_Model3D *model;
  S3L_Index modelIndex;
  ...
  modelIndex = 0;
    ...
    for ( S3L_Index triangleIndex = 0 ; triangleIndex < triangleCount ; triangleIndex++ )
    {
        S3L_Index vertexIndex0 = model->triangles[triangleIndex * 3 + 0];
        S3L_Index vertexIndex1 = model->triangles[triangleIndex * 3 + 1];
        S3L_Index vertexIndex2 = model->triangles[triangleIndex * 3 + 2];
        if ( ( vertexIndex0 < MAX_TRANSFORMED_VERTEX_COUNT ) && ( vertexIndex1 < MAX_TRANSFORMED_VERTEX_COUNT ) && ( vertexIndex2 < MAX_TRANSFORMED_VERTEX_COUNT ) )
        {
            if ( S3L_triangleIsVisible( transformed_vertex[vertexIndex0],
                                        transformed_vertex[vertexIndex1],
                                        transformed_vertex[vertexIndex2], model->config.backfaceCulling ) )
            {
                // without sorting draw right away
                S3L_drawTriangle( transformed_vertex[vertexIndex0],
                                  transformed_vertex[vertexIndex1],
                                  transformed_vertex[vertexIndex2], modelIndex, triangleIndex, u8_page_index );
            }
        }
    }
}

void S3L_drawScene(S3L_Scene scene, uint8_t u8_page_index)
{
  S3L_Mat4 matFinal, matCamera;
